...
import { Layers, Plus, Trash2, Search, CheckCircle2, AlertCircle } from "lucide-react"
...
export function WholesaleProductsView({ initialProducts, filterType }: WholesaleProductsViewProps) {
  ...
  const handleAddTier = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        setFeedback({ type: "error", text: "Failed to add tier" })
      }
  ...
  const handleDeleteTier = (productId: string | number, tierId: number) => {
    startTransition(async () => {
      ...
      setFeedback({ type: "success", text: "Wholesale tier removed" })
      setTimeout(() => setFeedback(null), 3000)
    })
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            ...
            Wholesale Products & Quantity Tiers
          </h1>
          <p className="text-xs text-gray-500">B2B volume discount brackets for bulk commercial purchasers</p>
        </div>
      ...
      <div className="relative max-w-md">
        ...
        <input
          type="text"
          placeholder="Filter wholesale products by title..."
          value={search}
                ...
                <tr key={item.id} className="hover:bg-gray-50/60 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-gray-400">{item.sellerName || "In-House"}</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-4">
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {item.tiers.length === 0 && (
                        <span className="text-gray-400 italic text-[11px]">No active brackets</span>
                      )}
                    </div>
                  </td>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          ...
          >
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Tier Price ($)</label>
              <input
                type="number"
                step="0.01"
                min={0}
