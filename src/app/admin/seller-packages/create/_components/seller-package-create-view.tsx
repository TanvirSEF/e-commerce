...
import { createSellerPackageAction } from "@/app/actions/ecommerce-actions"

export function SellerPackageCreateView() {
  const router = useRouter()
  const [name, setName] = useState("")
  const [amount, setAmount] = useState("0")
  const [productUploadLimit, setProductUploadLimit] = useState(100)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        await createSellerPackageAction({
          name: name.trim(),
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">Configure a subscription tier with upload quota and duration</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6 shadow-xs space-y-4">
        <div>
          ...
          <input
            type="text"
            required
            placeholder="e.g. Gold Platinum Pro"
            value={name}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-200 px-3.5 py-2 text-xs text-gray-900 focus:border-[#d43533] focus:outline-hidden"
            />
            <p className="text-[11px] text-gray-400 mt-1">Set 0 for a free starter package</p>
          </div>
